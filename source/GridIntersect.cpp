...
public:
  ...
  GridIntersectImpl(tbox::Pointer<hier::Patch<3> > source_patch) {
    ...
    int edge_number =
        source_patch->getNumberOfEntities(hier::EntityUtilities::EDGE, 0);
    int face_number =
    ...
    for (std::unordered_map<int, CellDescriptor>::iterator it = patch_cells_.begin();
  ...
  ~GridIntersectImpl() {
    for (std::unordered_map<int, NodeDescriptor*>::iterator iter = 
          patch_nodes_.begin(); iter != patch_nodes_.end(); iter++) 
      delete iter->second;
  }
...
#endif
          // std::unordered_map<int, NodeDescriptor>::iterator iter =
          //     patch_nodes_.find(face_nodes[i]);
          const hier::DoubleVector<3>& node_ref = 
...
private:
  std::unordered_map<int, NodeDescriptor*> patch_nodes_;  //  patch内节点集合
